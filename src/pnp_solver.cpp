		  ...
		  Eigen::Vector4d &Betas)
{
  ...
  L_6x4.col(0) = L_6x10.block<6, 1>(0, 0);
  L_6x4.col(1) = L_6x10.block<6, 1>(0, 1);
  L_6x4.col(2) = L_6x10.block<6, 1>(0, 3);
  L_6x4.col(3) = L_6x10.block<6, 1>(0, 6);
		  ...
		  Eigen::Vector4d &Betas)
{
  ...
  L_6x3.col(0) = L_6x10.block<6, 1>(0, 0);
  L_6x3.col(1) = L_6x10.block<6, 1>(0, 1);
  L_6x3.col(2) = L_6x10.block<6, 1>(0, 2);
		  ...
		  Eigen::Vector4d &Betas)
{
  ...
  L_6x5.col(0) = L_6x10.block<6, 1>(0, 0);
  L_6x5.col(1) = L_6x10.block<6, 1>(0, 1);
  L_6x5.col(2) = L_6x10.block<6, 1>(0, 2);
  L_6x5.col(3) = L_6x10.block<6, 1>(0, 3);
  L_6x5.col(4) = L_6x10.block<6, 1>(0, 4);
...
double PnpSolver::reprojection_error(const Eigen::Matrix3d &R,const Eigen::Vector3d &T)
{
  ...
  for(int i=0;i<number_of_correspondences;i++)
  {
    ...
    sum2+=(us.row(i).transpose()-point2d).norm();
  }
...
double PnpSolver::compute_pose(Eigen::Matrix3d &R,Eigen::Vector3d &T)
{
  ...
  MtM=M.transpose()*M;
  Eigen::JacobiSVD<Eigen::Matrix<double,12,12> > svd(MtM,ComputeFullU);
...
void PnpSolver::project_oints(std::vector<Eigen::Vector3d>& src,
...
void PnpSolver::gauss_newton(const Eigen::Matrix<double, 6, 10>& L_6x10,
                                             ...
                                             Eigen::Matrix<double, 6, 1>&        b)
{
    for (int i = 0; i < 6; ++i)
    {
        ...
        b[i]     = Rho[i] - (rowL[0] * betas[0] * betas[0] + rowL[1] * betas[0] * betas[1] +
...
void PnpSolver::qr_solve(Eigen::Matrix<double, 6, 4>& A,
                         Eigen::Matrix<double, 6, 1>& B,
                         Eigen::Vector4d&             X)
{
